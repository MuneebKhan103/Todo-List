...
taskDiv.innerHTML = `
  <input type="text" class="editField" id="editInput${index}" value="${currentValue.task}" />
    <div class="saveAndCancel">
      <button id = "saveBtn" onclick="saveEdit(${index})">Save</button>
      <button id= "cancel" onclick="display()">Cancel</button>
    </div>
  `;
};
const saveEdit = (index) => {
  const editComplete = document.getElementById(`editInput${index}`).value.trim();
    if (editComplete === "") {
    alert("You must write something!");
    return;
  }
taskToBeAdd[index].task = editComplete;
saveToLocal();
display();
};
const saveToLocal = () => {
